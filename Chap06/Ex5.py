...
class Panorama(QMainWindow) :
    def __init__(self) :
        ...
        self.removeBlackButton=QPushButton('검은색 영역 제거', self)
        ...
        self.removeBlackButton.setGeometry(310,25,100,30)
        ...
        self.removeBlackButton.setEnabled(False)
        ...
        self.removeBlackButton.clicked.connect(self.removeBlackFunction)
    ...
    def stitchFunction(self):
        ...
        if status==cv.STITCHER_OK:
            ...
            self.removeBlackButton.setEnabled(True)
        else:
            winsound.Beep(3000,500)            
            self.label.setText('파노라마 제작에 실패했습니다. 다시 시도하세요.')   
    
    def removeBlackFunction(self):
        if hasattr(self, 'img_stitched'):
            h, w = self.img_stitched.shape[:2]
            mask = np.zeros((h, w), np.uint8)
            cv.ellipse(mask, (int(w/2), int(h/2)), (int(w/2.5), int(h/2.5)), 0, 0, 360, (255, 255, 255), -1)
